...
users      Create a Matrix user with only a username prompt and a random 16-character password
admin      Create a Matrix administrator
cert       Obtain/refresh Let's Encrypt certificate and re-render Nginx/Coturn
rotate-passwords  Rotate PostgreSQL, Redis, Synapse, and TURN secrets
...
Create a standard Matrix user by entering only a username; LAZIESTMATRIX generates a strong 16-character password automatically and prints the account details in a copy-friendly table.

```bash
sudo ./laziestmatrix users
```

Rotate service credentials and shared secrets for PostgreSQL, Redis, Synapse, and Coturn with a copy-friendly table of the newly generated values:
